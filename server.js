...
function normalizeEtsyAuth(auth = {}) {
  const source = auth && typeof auth === "object" ? auth : {};
  const connected = Boolean(source.connected ?? (source.access_token || source.refresh_token || source.access_token_exists));
  const normalized = {
    connected,
    shop_id: String(source.shop_id || ""),
    shop_name: String(source.shop_name || ""),
    shop_url: String(source.shop_url || ""),
    access_token_exists: Boolean(source.access_token_exists || source.access_token),
    configured: source.configured ?? etsyConfigured(),
    expired: Boolean(source.expired),
    scopes: source.scopes || source.scope || ETSY_SCOPES,
    reconnect_required: Boolean(source.reconnect_required),
    token_status: source.token_status || (connected ? "active" : "not_connected"),
    source: source.source || (connected ? "normalized" : "none"),
    error: source.error || "",
    draft_safe: source.draft_safe ?? true
  };
  console.log("[AUTH NORMALIZED]", {
    connected: normalized.connected,
    shop_id: normalized.shop_id,
    shop_name: normalized.shop_name,
    access_token_exists: normalized.access_token_exists,
...
async function discoverEtsyShop(tokens, context = {}) {
  ...
  await writeEtsyTokens(updated);
  if (context.req?.session) {
    console.log("[SHOP DISCOVERY]", {
      shop_id: updated.shop_id,
      shop_name: updated.shop_name,
      source: "discoverEtsyShop"
    });
...
function sendFailureReason(error = {}) {
  ...
  if (code === "shop_mismatch") return "shop_mismatch";
...
function extractSingleEtsyListing(payload = {}) {
...
function etsyListingShopId(listing = {}) {
  return normalizeListingId(listing.shop_id || listing.Shop?.shop_id || listing.shop?.shop_id || "");
}

function etsyListingIsLive(listing = {}) {
  const state = String(listing.state || listing.status || "active").trim().toLowerCase();
  return !["inactive", "expired", "sold_out", "removed", "deleted", "draft"].includes(state);
}

function etsyListingState(listing = {}) {
  return String(listing.state || listing.status || "").trim();
}

function etsyVerifyReason(result = {}) {
  if (result.found) return "accepted";
  if (result.shopMatch === false) return "shop_mismatch";
  if (result.active === false) return "not_active";
  if (Number(result.status || 0) === 404) return "not_found";
  if (Number(result.status || 0) === 401 || Number(result.status || 0) === 403) return "auth_or_permission";
...
async function pruneListingFromCache(listingId = "", shopId = "") {
  ...
  const cachedListings = await readListingsCache(shopId);
  const nextListings = cachedListings.filter((listing) => normalizeListingId(listing.listing_id || listing.id) !== normalizedListingId);
  if (nextListings.length !== cachedListings.length) {
    const meta = await readListingsMeta();
    await writeListingsCache(nextListings, {
      ...meta,
      shop_id: shopId || meta.shop_id || "",
      source: meta.source || "etsy_api",
      stale_removed_listing_id: normalizedListingId
    });
    console.log("[LISTING REMOVED FROM CACHE]", { listing_id: normalizedListingId, shop_id: shopId || meta.shop_id || "" });
  }
}
...
async function resolveLiveListingForEtsyPut(product = {}, shopId = "") {
  ...
  if (requestedId) return { listing_id: requestedId, matched: "request_id" };
...
async function safeEtsyStatus(tokens = {}) {
  ...
  return normalizeEtsyAuth(status && typeof status === "object" ? status : {});
}
...
async function directVerifyEtsyListing(tokens = {}, shopId = "", listingId = "") {
  ...
  for (const { type, key, endpoint } of endpoints) {
    ...
    const listing = extractSingleEtsyListing(body);
    const responseShopId = etsyListingShopId(listing);
...
async function verifyEtsyListingForSend(tokens = {}, shopId = "", listingId = "") {
  return directVerifyEtsyListing(tokens, shopId, listingId);
}
...
async function updateEtsyListingDirect(req, res, product = {}) {
  ...
  console.log("[SEND SHOP CHECK]", {
    listing_id: listingId,
    shop_id: shopId,
    token_shop_id: normalizeListingId(tokens.shop_id || ""),
    session_shop_id: normalizeListingId(sessionShopId || ""),
    session_auth_shop_id: normalizeListingId(sessionAuthShopId || ""),
    request_auth_shop_id: normalizeListingId(requestAuthShopId || "")
  });
  if (!/^\d+$/.test(shopId)) {
  ...
  const preUpdateVerify = await verifyEtsyListingForSend(tokens, shopId, liveListingId);
  let verifyEndpoint = preUpdateVerify.endpoint || `${etsyUpdateBase}${listingPath}`;
  let verifyPayload = preUpdateVerify.body || {};
  console.log("[ETSY VERIFY LISTING RESPONSE]", {
    status: preUpdateVerify.status,
    body: verifyPayload,
    verify_endpoint_used: preUpdateVerify.endpoint || "",
    verify_state: preUpdateVerify.verify_state || "",
    verify_shop_id: preUpdateVerify.responseShopId || "",
    verify_reason: etsyVerifyReason(preUpdateVerify)
  });
  if (!preUpdateVerify.found) {
    if (Number(preUpdateVerify.status || 0) === 404) {
    ...
    const retryVerify = Number(preUpdateVerify.status || 0) === 404 ? await verifyEtsyListingForSend(tokens, shopId, liveListingId) : preUpdateVerify;
    if (!retryVerify.found) {
      if (Number(retryVerify.status || 0) === 404) await pruneListingFromCache(liveListingId, shopId);
      const error = new Error(retryVerify.shopMatch === false ? "Listing belongs to a different Etsy shop." : "This listing is not currently active/live in your Etsy shop. Refresh Etsy Listings.");
      error.code = retryVerify.shopMatch === false ? "shop_mismatch" : "etsy_resource_not_found";
      error.status = retryVerify.shopMatch === false ? 409 : Number(retryVerify.status || 404);
      error.payload = {
        shop_id: shopId,
        listing_id: liveListingId,
        etsy_status: retryVerify.status,
        etsy_body: retryVerify.body,
    ...
    verifyEndpoint = retryVerify.endpoint || verifyEndpoint;
    verifyPayload = retryVerify.body || {};
  }
  ...
  const currentListing = Array.isArray(verifyPayload?.results)
    ...
    : verifyPayload;
  const currentShopId = etsyListingShopId(currentListing);
  const currentShopMatches = !currentShopId || currentShopId === shopId;
  const currentListingLive = etsyListingIsLive(currentListing);
  console.log("[SEND VERIFY RESULT]", {
    listing_id: liveListingId,
    status: preUpdateVerify.status,
    found: preUpdateVerify.found,
    shopMatch: currentShopMatches,
    active: currentListingLive,
    responseShopId: currentShopId
  });
  if (!currentShopMatches) {
    console.warn("[SEND BLOCKED SHOP_MISMATCH]", { listing_id: liveListingId, shop_id: shopId, response_shop_id: currentShopId });
    const error = new Error("Listing belongs to a different Etsy shop.");
    ...
    error.payload = {
      ...
      verify_reason: "shop_mismatch"
    };
    throw error;
  }
  if (!currentListingLive) {
    console.warn("[SEND BLOCKED NONLIVE]", { listing_id: liveListingId, shop_id: shopId, state: currentListing?.state || currentListing?.status || "" });
    await pruneListingFromCache(liveListingId, shopId);
...
app.get("/api/etsy/status", async (req, res) => {
  try {
    ...
    const etsy = await safeEtsyStatus(tokens);
    res.json(successResponse({ ...etsy, etsy }, "Etsy auth status loaded"));
  } catch (error) {
    res.status(500).json(errorResponse("etsy_status_failed", error instanceof Error ? error.message : String(error), { etsy: normalizeEtsyAuth({ error: "etsy_status_failed" }) }));
  }
});
...
async function handleActivateEtsyToken(req, res) {
  ...
  if (!email || !token) {
    ...
    res.status(400).json(errorResponse("connect_token_invalid_or_expired", "Etsy connect token is invalid or expired.", {
      ...
      etsy: normalizeEtsyAuth({ reconnect_required: true, error: "connect_token_invalid_or_expired" })
    }));
    return;
  }
  const users = await readUsers();
  ...
  const etsy = normalizeEtsyAuth(await safeEtsyStatus(user.etsy_auth));
  res.json(successResponse({
    status: "activated",
    connected: true,
    shop_id: etsy.shop_id,
    shop_name: etsy.shop_name,
...
app.post("/api/etsy/disconnect", requireUser, async (req, res) => {
  ...
  res.json(successResponse({ etsy: normalizeEtsyAuth({ connected: false }) }, "Etsy disconnected"));
});

app.post("/api/etsy/sync", requireUser, async (req, res) => {
  try {
    if (!etsyConfigured()) {
      res.status(503).json(errorResponse("etsy_not_configured", "Etsy API is not configured.", { etsy: normalizeEtsyAuth({ error: "etsy_not_configured" }) }));
      return;
    }
    const userTokens = await resolveRequestEtsyAuth(req);
    if (!userTokens.access_token) {
      res.status(401).json(errorResponse("etsy_not_connected", "Please connect your Etsy shop", { listings: [], etsy: normalizeEtsyAuth(await safeEtsyStatus(userTokens)) }));
      return;
    }
    ...
    res.json(successResponse({
      ...
      source: "etsy_api",
      listings,
      etsy: normalizeEtsyAuth(await safeEtsyStatus(req.etsyAuth || userTokens))
    ...
    res.status(status).json(errorResponse(
      ...
      { etsy: normalizeEtsyAuth({ ...(await safeEtsyStatus(req.etsyAuth || {})), reconnect_required: true, token_status: "reconnect_required", error: errorCode }) }
    ));
  }
});
...
app.post("/api/etsy/refresh-sync", requireUser, async (req, res) => {
  try {
    ...
    res.json(successResponse({
      ...
      listings,
      etsy: normalizeEtsyAuth(await safeEtsyStatus(token))
...
app.get("/api/listings", requireUser, async (req, res) => {
  try {
    const userTokens = await resolveRequestEtsyAuth(req);
    console.log("[LISTINGS AUTH STATE]", normalizeEtsyAuth(userTokens));
    ...
    if (!userTokens.access_token) {
      res.status(401).json(errorResponse("etsy_not_connected", "Please connect your Etsy shop", {
        status: "failed",
        listings: [],
        etsy: normalizeEtsyAuth(await safeEtsyStatus(userTokens))
      }));
      return;
    }
    ...
    res.status(status).json(errorResponse(errorCode, error instanceof Error ? error.message : String(error), {
      status: "failed",
      error: error instanceof Error ? error.message : String(error),
      etsy: normalizeEtsyAuth({ error: errorCode, reconnect_required: status === 401 })
    }));
  }
});
  ...
  try {
    ...
    for (const product of products) {
      ...
      if (!listingId || !/^\d+$/.test(listingId) || ["none", "undefined", "null"].includes(String(listingId).toLowerCase())) {
        skipped += 1;
        console.warn("[SEND BLOCKED NONLIVE]", { listing_id: listingId || "missing", reason: "invalid_listing_id" });
      ...
      if (directEtsyMode && (!liveIds.has(listingId))) {
        ...
        console.log("[SEND VERIFY RESULT]", {
          ...
          found: Boolean(directVerify.found),
          shopMatch: directVerify.shopMatch,
          active: directVerify.active,
          responseShopId: directVerify.responseShopId || ""
        });
        if (directVerify.found) {
          ...
          if (verifyStatus === 404 || directVerify.shopMatch === false || directVerify.active === false) {
            await pruneListingFromCache(listingId, shopId);
            skipped += 1;
            const blockedReason = directVerify.shopMatch === false ? "shop_mismatch" : "not_live_listing";
            console.warn(directVerify.shopMatch === false ? "[SEND BLOCKED SHOP_MISMATCH]" : "[SEND BLOCKED NONLIVE]", {
              listing_id: listingId,
              shop_id: shopId,
              response_shop_id: directVerify.responseShopId || "",
              active: directVerify.active,
              etsy_status: directVerify.status
            });
            console.warn("[SEND BATCH ID MATCH CHECK]", { listing_id: listingId, matched: false, reason: blockedReason, etsy_status: directVerify.status });
            console.warn("[SEND DECISION]", { listing_id: listingId, decision: "skipped_not_live" });
            console.warn("[SEND SELECTED SKIPPED]", { listing_id: listingId, reason: blockedReason });
      ...
      console.log("[SEND DECISION]", { listing_id: listingId, decision: "send_to_etsy" });
      ...
      try {
        await incrementAnalytics("optimization_started");
        console.log("[ETSY PUT READY]", { listing_id: listingId, fields });
        ...
        if (error?.code === "etsy_resource_not_found" || error?.code === "stale_listing_id" || error?.code === "shop_mismatch") {
          ...
          console.warn("[SEND SELECTED SKIPPED]", { listing_id: listingId, reason: skippedReason });
          console.warn("[SEND SELECTED SKIP REASON]", {
            listing_id: listingId,
            reason: skippedReason,
            liveListingIds_count: liveIds.size,
        ...
        results.push({
          ...
          verify_state: error?.payload?.verify_state || "",
          verify_shop_id: error?.payload?.verify_shop_id || "",
          verify_reason: error?.payload?.verify_reason || reason
        });
        results[results.length - 1] = normalizeSendResult(results[results.length - 1]);
      }
    }
